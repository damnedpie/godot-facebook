package com.onecat.godotfacebook;
...
public class GodotFacebook extends GodotPlugin {
		...
		getActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				try {
					...
					fbLogger = AppEventsLogger.newLogger(getActivity().getApplicationContext());
	...
	public String[] extinfo() {
		...
		try {
			PackageInfo pi = getActivity().getApplicationContext().getPackageManager().getPackageInfo(pkgName, 0);
